window.addEventListener('load', () =>{
    ...
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition( async position => {
            ...
            const lat = position.coords.latitude;   // latitude y
            const long = position.coords.longitude; // longitude x
            ...
            const res_current_address = await fetch(`/address/${lat},${long}`);
            const current_address = await res_current_address.json();
            
            const res_current_weather = await fetch(`/current_weather/${lat},${long}`);
            const current_weather = await res_current_weather.json();

            const res_forecast = await fetch(`/forecast/${lat},${long}`);
            const forecast = await res_forecast.json();

            const res_transform = await fetch(`/transform/${lat},${long}`);
            const transform = await res_transform.json();
            const tmX = transform.tmX;
            const tmY = transform.tmY;

            const res_near_location = await fetch(`/near_location/${tmX},${tmY}`);
            const near_location = await res_near_location.json();
            const near_stationName = near_location.stationName

            const res_current_aq_info = await fetch(`/current_aq_info/${near_stationName}`);
            const current_aq_info = await res_current_aq_info.json();
        
            // Set current weather section
            setCurrentWeather(current_weather.current_weather, current_address.result_address);

            // Set forecast section
            setForecast(forecast.weather_forecast);


            // Set current air quality section 
            const {dataTime, pm10Value, pm25Value, o3Value, no2Value, coValue, so2Value, pm10value24, pm25Value24} = current_aq_info.current_aq_info.list[0];
            setFineDust(current_address.result_address, near_stationName, dataTime, pm10Value, pm25Value, o3Value, no2Value, coValue, so2Value, pm10value24, pm25Value24);
    ...
    function setFineDust(currentAddress, stationName, dataTime, pm10Value, pm25Value, o3Value, no2Value, coValue, so2Value, pm10value24, pm25Value24){
        // pm-location
